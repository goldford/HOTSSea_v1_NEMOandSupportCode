''' convert DFO IOS Nanoose Stn (military) files
    G Oldford Jan 2023
    
    Work in progress - must revert var dim names to what pyap requires
                     - dims: pressure, single
                     - vars: Pres(pressure), temperature(pressure), cTemp(pressure)
                             salinity(pressure), aSal(pressure), pDen(pressure)
                             time(single), depth(single), latitude(single), longitude(single)
    
    Based on code in python-analysis-package etc/data_conversion_functions.py 
    data in: many CTD and MED files in many folders from Nanoose Station (data from MD Aug 2022)
...
indir = '/project/6006412/goldford/data/evaluation/nanoose_stn/NANOOSE/**/'
saveDir = '/project/6006412/goldford/data/evaluation/nanoose_stn/prepped_pre2003_pyapnames/'
os.makedirs(saveDir, exist_ok=True)

# this is not working - it is grabbing all files not just MED and CTD
files_MED_CTD = sorted(glob.glob(os.path.join(indir, "*.[MED CTD]*"), recursive=True)) 
...
## main loop - added both types of files jan 2023 - GO
for file in files_MED_CTD: 
...
#      print("no water dep found. continuing anyway.") 
    ...
    if st_check == 0:
      ...
      print("time increment field found indicating newer file format. skipping for now.")
      continue
    else:
...
#
    ...
    if (TChan == -1) and (TChan_low == -1) and (TChan_high == -1):
      ...
      continue 
    # salin catch disabled MED files have no salin - GO 2023
    #elif (SChan == -1):
    #  print("no salin channel. continuing anyway.")
      #continue 
    elif (PChan == -1):
    ...
    for n,line in enumerate(lines[skipheader:]):
      ...
      # disabling b/c MED files have empty S - GO Jan 2023
      #else:
        #print("no S found. proceeding anyway.")
        #continue
    ...
    # disabling catch for empty salt to handle MED files
    if all(np.isnan(S)):
        flags['S'] = False
...
#    print("found startTime - len: ", sT)
    
    # added to handle the MED files which have no salinity - GO Jan 2023
    if SChan != -1:
        T, S, P, badvals = general_utils.remove_nan(T, S, P, badvals) # removes nans across all if one of T,S,P has nan
        ...
        S = np.full_like(T, -999)
        absSal = np.full_like(T, -999)
        consTemp = T # is very close to same for salt water
        pDen = np.full_like(T, -999)
...
#    print("mean Salin: ", np.mean(S))
    ...
    #CTD_Cast(self, saveDir, station, startT, P, T, consTemp, S, absSal, pDen, dep, lat, lon)
    outfile = write_to_netcdf_GO().CTD_Cast(saveDir, stationstr, sT, P, T, consTemp, S, absSal, pDen, dep, lat, lon)
