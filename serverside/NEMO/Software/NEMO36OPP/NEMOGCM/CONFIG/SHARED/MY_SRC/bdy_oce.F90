MODULE bdy_oce
   ...
   LOGICAL                    ::   ln_mask_file = .FALSE.   !: =T read bdymask from file
   LOGICAL                    ::   ln_vol = .FALSE.         !: =T volume correction
   !
   INTEGER                    ::   nb_bdy = 0               !: number of open boundary sets
   INTEGER, DIMENSION(jp_bdy) ::   nn_rimwidth              !: boundary rim width for Flow Relaxation Scheme
   INTEGER                    ::   nn_volctl = 0            !: = 0 the total volume will have the variability of the surface Flux E-P
...
#if defined key_opp
   REAL(wp),    DIMENSION(jp_bdy) :: rn_ssh_offset          !: SSH offset for the boundary (in m up)
#endif
