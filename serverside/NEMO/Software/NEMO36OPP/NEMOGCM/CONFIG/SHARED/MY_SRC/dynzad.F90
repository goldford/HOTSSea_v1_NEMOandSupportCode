...
#if defined key_opp
   SUBROUTINE dyn_zad_zts ( kt, jnzts )
#else
   SUBROUTINE dyn_zad_zts ( kt )
...
#if defined key_turbo
      zjpkm1 = jpkm1  ! Store jpkm1
      jpkm1 = MIN(MAXVAL(mbkt) + 1, jpkm1) ! Reduce jpkm1
#endif
      
      DO jk = 2, jpkm1                    ! Calculate and store vertical fluxes
         DO jj = 2, jpj                   
            DO ji = fs_2, jpi             ! vector opt.
               zww(ji,jj,jk) = 0.25_wp * e12t(ji,jj) * wn(ji,jj,jk)
            END DO
         END DO
      END DO
      !
      ...
      DO jj = 2, jpjm1        
         DO ji = fs_2, fs_jpim1           ! vector opt.
            ...
            zwvw(ji,jj, 1 ) = 0._wp
#if defined key_turbo
            zwuw(ji,jj,jpkm1:jpk) = 0._wp
            zwvw(ji,jj,jpkm1:jpk) = 0._wp
#else
            zwuw(ji,jj,jpk) = 0._wp
            zwvw(ji,jj,jpk) = 0._wp
#endif
...
! Start with before values and use sub timestepping to reach after values
      ...
      DO jl = 1, jnzts                   ! Start of sub timestepping loop
         ...
         DO jk = 1, jpkm1           ! Vertical momentum advection at u- and v-points
            DO jj = 2, jpjm1
               DO ji = fs_2, fs_jpim1       ! vector opt.
                  !                         ! vertical momentum advective trends
                  zua = - ( zwuw(ji,jj,jk) + zwuw(ji,jj,jk+1) ) * r1_e12u(ji,jj) / fse3u(ji,jj,jk)
                  zva = - ( zwvw(ji,jj,jk) + zwvw(ji,jj,jk+1) ) * r1_e12v(ji,jj) / fse3v(ji,jj,jk)
      ...
      !
#if defined key_turbo
      jpkm1 = zjpkm1 ! Restore saved value of jpkm1
#endif
