...
CONTAINS
   ...
   SUBROUTINE tra_adv_tvd_zts ( kt, kit000, cdtype, p2dt, pun, pvn, pwn,      &
      ...
      !
#if defined key_turbo
      zjpkm1 = jpkm1  ! Save the value of jpkm1
      ...
      DO jn = 1, kjpt                                            ! tracer loop
         ...
         ! ----------------------------------
#if defined key_turbo
         zwx(:,:,jpkm1:jpk) = 0._wp   ;    zwz(:,:,jpkm1:jpk) = 0._wp
         zwy(:,:,jpkm1:jpk) = 0._wp   ;    zwi(:,:,jpkm1:jpk) = 0._wp
#else
         ...
         DO jk = 1, jpkm1
            ...
            DO jj = 2, jpjm1
               DO ji = fs_2, fs_jpim1   ! vector opt.
                  ...
                  ztra = - (  zwx(ji,jj,jk) - zwx(ji-1,jj  ,jk  )   &
                     &      + zwy(ji,jj,jk) - zwy(ji  ,jj-1,jk  )   &
                     &      + zwz(ji,jj,jk) - zwz(ji  ,jj  ,jk+1) ) * r1_e12t(ji,jj)
                  ! update and guess with monotonic sheme
                  pta(ji,jj,jk,jn) =                       pta(ji,jj,jk,jn) +         ztra   / fse3t_n(ji,jj,jk) * tmask(ji,jj,jk)
                  zwi(ji,jj,jk)    = ( fse3t_b(ji,jj,jk) * ptb(ji,jj,jk,jn) + z2dtt * ztra ) / fse3t_a(ji,jj,jk) * tmask(ji,jj,jk)
               END DO
      ...
      DO jk = 1, jpkm1
         ...
         DO jj = 2, jpjm1
            DO ji = fs_2, fs_jpim1   ! vector opt.
               ...
               ! up & down beta terms
               zbt = e12t(ji,jj) * fse3t(ji,jj,jk) / z2dtt
