...
#if defined key_opp
         CASE('ssh')
            CALL bdy_dyn2d_ssh( idx_bdy(ib_bdy), dta_bdy(ib_bdy), ib_bdy, pua2d, pva2d, pssh, phur, phvr )
#endif
         CASE('orlanski')
...
#if defined key_opp
   SUBROUTINE bdy_dyn2d_ssh ( idx, dta, ib_bdy, pua2d, pva2d, pssh, phur, phvr )
      !!----------------------------------------------------------------------
...
!!! REPLACE spgu with nemo_wrk work space
      ...
      DO jb = 1, idx%nblenrim(igrd)
         ...
         if (INT(flagu) == -1) then
             pua2d(ii,ij) = pua2d(iim1-1,ij) * umask(ii,ij,1)
             pssh(iim1, ij) = spgu(iip1,ij)*tmask(iim1,ij,1)
         endif
      END DO
      !
      ...
      DO jb = 1, idx%nblenrim(igrd)
         ...
         if(INT(flagv) == -1) then
             pva2d(ii,ij) = pva2d(ii,ijm1-1) * vmask(ii,ij,1)
             pssh(ii, ijm1) = spgu(ii,ijp1)*tmask(ii,ijm1 ,1)
         endif
      END DO
...
!         ii = idx%nbi(jb,igrd)
!         ij = idx%nbj(jb,igrd)
!         pssh(ii, ij) = dta%ssh(jb)
!      END DO
      CALL lbc_bdy_lnk( pssh(:,:), 'T', 1., ib_bdy )
      !
