...
CONTAINS
   ...
   SUBROUTINE istate_init
      ...
      USE sshwzv          ! vertical velocity and ssh             (wzv and ssh_nxt routines)
      !
      REAL(wp), POINTER, DIMENSION(:,:,:)   ::   zssh    ! SSH data workspace
      REAL(wp)::   zcoef
#endif
      ...
      ELSE
         ...
         !
#if defined key_opp
         !                              ! allocate sbc arrays (moved here from sbcmod.F90)
         IF(ln_rstart .eqv. .false.) THEN
...
#if defined key_opp
            IF ( ln_uvd_init ) THEN              ! read 3D U and V data and 2D SSH data at nit000
               CALL wrk_alloc( jpi, jpj, jpk, 2, zuvd )
               CALL wrk_alloc( jpi, jpj, 1, zssh )
               CALL dta_uvd( nit000, zuvd, zssh )
               ub(:,:,:) = zuvd(:,:,:,1) ;  un(:,:,:) = ub(:,:,:)
               vb(:,:,:) = zuvd(:,:,:,2) ;  vn(:,:,:) = vb(:,:,:)
               sshb(:,:) = zssh(:,:,1) ;  sshn(:,:) = sshb(:,:)
               CALL wrk_dealloc( jpi, jpj, jpk, 2, zuvd )
               CALL wrk_dealloc( jpi, jpj, 1, zssh )
            ENDIF
#endif
         ...
         ENDIF
#else
! Since the model is initialized with SSH data, the vertical scale factors need
! to be updated. The following code was copied from dom_vvl_rst and dom_vvl_init
         IF( lk_vvl ) THEN
            IF(lwp) write(numout,*) 'Compute vertical scale factor from sshn'
             DO jk=1,jpk
             ...
             END DO
             !
             ! Before (and now) depth and Inverse of the local depth of the water column at u- and v- points
             ! -----------------------------------------------------------------------------------
             hu(:,:) = 0.
             hv(:,:) = 0.
             ht(:,:) = 0.
         ...
         ENDIF
         !
         CALL ssh_nxt( nit000 )                   ! first guess ssha (includes call to div_cur routine -> hdivn, rotn)
         hdivb(:,:,:) = hdivn(:,:,:)
         rotb (:,:,:) = rotn (:,:,:)
         !
         CALL dom_vvl_sf_nxt( nit000 )            ! computes fse3t_a, fse3u_a, fse3v_a
         IF( .NOT. ln_rstart ) CALL wzv( nit000 ) ! now vertical velocity
         !
