MODULE zdfbfr
   ...
   !!            3.6  ! 2016-12  (S. Allen) split bottom-friction to u and v for jetty
#endif
   !!----------------------------------------------------------------------
...
#if defined key_salishsea
   REAL(wp), ALLOCATABLE, SAVE, DIMENSION(:,:), PUBLIC ::  bfrcoef2du, bfrcoef2dv, tfrcoef2d   ! 2D bottom/top drag coefficient (PUBLIC for TAM)
#else
...
CONTAINS
   ...
   INTEGER FUNCTION zdf_bfr_alloc()
      ...
      !!----------------------------------------------------------------------
#if defined key_salishsea
      ALLOCATE( bfrcoef2du(jpi,jpj), bfrcoef2dv(jpi, jpj), tfrcoef2d(jpi,jpj), STAT=zdf_bfr_alloc )
#else
      ...
      IF( nn_bfr == 2 ) THEN                 ! quadratic bottom friction only
         ...
         IF ( ln_loglayer.AND.lk_vvl ) THEN ! "log layer" bottom friction coefficient
            ...
            DO jj = 1, jpj
               DO ji = 1, jpi
                  ikbt = mbkt(ji,jj)
                  IF (fse3t_n(ji, jj, ikbt) > 2*rn_bfrz0) then
                     zbfrt(ji,jj) = tmask(ji,jj,ikbt) * ( vkarmn / LOG( 0.5_wp * fse3t_n(ji,jj,ikbt) / rn_bfrz0 ))**2._wp
                  ELSE
                     zbfrt(ji, jj) = rn_bfri2_max
                  ENDIF
               END DO
            END DO
            DO jj = 1, jpjm1
               DO ji = 1, jpim1
                  ztmp = 0.5_wp *(zbfrt(ji,jj) + zbfrt(ji+1,jj) )
                  ztmp = MIN(rn_bfri2_max, ztmp)
                  bfrua(ji,jj) = MAX(bfrcoef2du(ji,jj), ztmp)
                  ztmp = 0.5_wp *(zbfrt(ji,jj) + zbfrt(ji,jj+1) )
                  ztmp = MIN(rn_bfri2_max, ztmp)
                  bfrva(ji,jj) = MAX(bfrcoef2dv(ji,jj), ztmp)
               END DO
            END DO
#else
            DO jj = 1, jpj
...
! JC: possible WAD implementation should modify line below if layers vanish
         ...
         DO jj = 2, jpjm1
            DO ji = 2, jpim1
               ...
               !
#if defined key_salishsea
               bfrua(ji,jj) = - bfrua(ji,jj) * zecu
               bfrva(ji,jj) = - bfrva(ji,jj) * zecv
#else
...
902   IF( ios /= 0 ) CALL ctl_nam ( ios , 'nambfr in configuration namelist', lwp )
      ...
      CASE( 1 )
         ...
         IF(ln_bfr2d) THEN
            ...
            CALL iom_get (inum, jpdom_data, 'bfr_coef_u',bfrcoef2du,1) ! bfrcoef2du is used as tmp array
            bfrcoef2du(:,:) = rn_bfri1 * ( 1 + rn_bfrien * bfrcoef2du(:,:) )
            CALL iom_get (inum, jpdom_data, 'bfr_coef_v',bfrcoef2dv,1) ! bfrcoef2dv is used as tmp array
            CALL iom_close(inum)
            bfrcoef2dv(:,:) = rn_bfri1 * ( 1 + rn_bfrien * bfrcoef2dv(:,:) )
         ELSE
            bfrcoef2du(:,:) = rn_bfri1  ! initialize bfrcoef2du to the namelist variable
            bfrcoef2dv(:,:) = rn_bfri1  ! initialize bfrcoef2dv to the namelist variable
         ENDIF
         !
         bfrua(:,:) = - bfrcoef2du(:,:)
         bfrva(:,:) = - bfrcoef2dv(:,:)
      ...
      CASE( 2 )
         ...
         IF(ln_bfr2d) THEN
            ...
            CALL iom_close(inum)
            !
            bfrcoef2du(:,:) = rn_bfri2 * ( 1 + rn_bfrien * bfrcoef2du(:,:) )
            bfrcoef2dv(:,:) = rn_bfri2 * ( 1 + rn_bfrien * bfrcoef2dv(:,:) )
         ELSE
            bfrcoef2du(:,:) = rn_bfri2  ! initialize bfrcoef2du to the namelist variable
            bfrcoef2dv(:,:) = rn_bfri2  ! initialize bfrcoef2dv to the namelist variable
         ENDIF
#else
         IF(ln_bfr2d) THEN
            ! bfr_coef is a coefficient in [0,1] giving the mask where to apply the bfr enhancement
         ...
         IF ( ln_loglayer.AND.(.NOT.lk_vvl) ) THEN ! set "log layer" bottom friction once for all
            ...
            DO jj = 1, jpjm1
               DO ji = 1, jpim1
                  ikbt = mbkt(ji,jj)
                  ztmp = tmask(ji,jj,ikbt) * ( vkarmn / LOG( 0.5_wp * fse3t_n(ji,jj,ikbt) / rn_bfrz0 ))**2._wp
                  ztmp = 0.5 * (ztmp + tmask(ji+1,jj,ikbt) * ( vkarmn / LOG( 0.5_wp * fse3t_n(ji+1,jj,ikbt) / rn_bfrz0 ))**2._wp)
                  ztmp = MIN(ztmp, rn_bfri2_max)
                  bfrcoef2du(ji,jj) = MAX(bfrcoef2du(ji,jj), ztmp)
                  ztmp = tmask(ji,jj,ikbt) * ( vkarmn / LOG( 0.5_wp * fse3t_n(ji,jj,ikbt) / rn_bfrz0 ))**2._wp
                  ztmp = 0.5 * (ztmp + tmask(ji,jj+1,ikbt) * ( vkarmn / LOG( 0.5_wp * fse3t_n(ji,jj+1,ikbt) / rn_bfrz0 ))**2._wp)
                  ztmp = MIN(ztmp, rn_bfri2_max)
                  bfrcoef2dv(ji,jj) = MAX(bfrcoef2dv(ji,jj), ztmp)
               END DO
            END DO
#else
            DO jj = 1, jpj
...
#if defined key_salishsea
             IF( ABS( bfrcoef2du(ji,jj) ) > zfru ) THEN
                IF( ln_ctl ) THEN
                   WRITE(numout,*) 'BFR ', narea, nimpp+ji, njmpp+jj, ikbu
                   WRITE(numout,*) 'BFR ', ABS( bfrcoef2du(ji,jj) ), zfru
                ENDIF
                ictu = ictu + 1
             ENDIF
             IF( ABS( bfrcoef2dv(ji,jj) ) > zfrv ) THEN
                 IF( ln_ctl ) THEN
                     WRITE(numout,*) 'BFR ', narea, nimpp+ji, njmpp+jj, ikbv
                     WRITE(numout,*) 'BFR ', bfrcoef2dv(ji,jj), zfrv
                 ENDIF
                 ictv = ictv + 1
             ENDIF
             zminbfr = MIN(  zminbfr, MIN( zfru, ABS( bfrcoef2du(ji,jj) ) )  )
             zmaxbfr = MAX(  zmaxbfr, MIN( zfrv, ABS( bfrcoef2dv(ji,jj) ) )  )
#else
