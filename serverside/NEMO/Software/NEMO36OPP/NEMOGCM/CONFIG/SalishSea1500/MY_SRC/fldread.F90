...
#if defined key_opp
      REAL(wp)                        ::   nsec_shift   ! When ln_tint=.true., NEMO assumes the records correspond to the centre of each nfreqh period.
                                                        ! For hourly data, this means the records are assumed to lie at 00:30, 01:30, ..., 23:30.
                                                        !
                                                        ! Meanwhile:
                                                        ! Hourly NEMO snapshots are saved at 01:00, 02:00, ..., 23:00, 24:00, and are typically split into daily files.
                                                        ! Similarly, hourly HRDPS forcing also takes the form of snapshots at the top of each hour, eg, 00:00, 01:00, ....
                                                        !
                                                        ! Thus we usually wish to remove this 30 min offset in the online temporal interpolation. There are two options:
                                                        !
                                                        ! If you construct daily forcing files with hourly records, and the records correspond to 01:00, 02:00, ..., 24:00,
                                                        ! then we need to apply a +0.5h shift. Do this by prefixing the variable name with +, eg, '+qair'.
                                                        !
                                                        ! If you construct daily forcing files with hourly records, and the records correspond to 00:00, 01:00, ..., 23:00,
                                                        ! then we need to apply a -0.5h shift. Do this by prefixing the variable name with -, eg, '-qair'.
#endif
   END TYPE FLD
...
#if defined key_opp
            ztmp= ztmp - sdjf%nsec_shift/REAL(ifreq_sec, wp)
#endif
         ELSE                                   ! no time interpolation
            !
            !           INT( ztmp/ifreq_sec )
...
#if defined key_opp
             sdjf%nrec_a(2) = sdjf%nrec_a(2) + INT(sdjf%nsec_shift)
#endif
         ELSE
...
#if defined key_opp
         sdf(jf)%nsec_shift = 0._wp
         IF (sdf_n(jf)%clvar(1:1) == '+') THEN
           sdf(jf)%nsec_shift =  0.5_wp*3600._wp*sdf(jf)%nfreqh
           sdf(jf)%clvar      = sdf_n(jf)%clvar(2:)
         ELSE IF (sdf_n(jf)%clvar(1:1) == '-') THEN
           sdf(jf)%nsec_shift = -0.5_wp*3600._wp*sdf(jf)%nfreqh
           sdf(jf)%clvar      = sdf_n(jf)%clvar(2:)
         ENDIF
#endif
