...
#sbatch --time=${TIMELIMIT} --job-name=$CONF_NBR --mem-per-cpu=${MEMLIMIT} --ntasks=${TOTPROC} --nodes=3  ${ACCOUNT} -o ${DIR_TMP_RUN}/nemo.stdout -e ${DIR_TMP_RUN}/nemo.stderr $CONF_NBR.ksh
