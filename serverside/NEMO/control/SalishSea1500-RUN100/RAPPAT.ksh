...
if [ "$CONF_NAME" == "SalishSea1500" ] ; then
  src=${HOME}/projects/rrg-nereusvc/mdunphy/Forcing/forcing_ciopswbc12_20210406
  ln -s ${src}/bdy_ssh bdy_ssh
  ln -s ${src}/bdy_ts bdy_ts
  ln -s ${src}/bdy_uv bdy_uv
  ln -s ${src}/ic_ssh ic_ssh
  ln -s ${src}/ic_ts ic_ts
  ln -s ${src}/ic_uv ic_uv
  ln -s ${HOME}/projects/rrg-nereusvc/mdunphy/Forcing/ss1500tides tides
  ln -s ${HOME}/projects/rrg-nereusvc/goldford/data/runoff/rivers_month_20210406_GO.nc .
  ln -s ${HOME}/projects/rrg-nereusvc/mdunphy/Forcing/daily_runoff rivers
fi
...
echo sbatch --time=${TIMELIMIT} --job-name=$CONF_NBR --mem-per-cpu=${MEMLIMIT} --ntasks=${TOTPROC} ${ACCOUNT} $CONF_NBR.ksh
sbatch --time=${TIMELIMIT} --job-name=$CONF_NBR --mem-per-cpu=${MEMLIMIT} --ntasks=${TOTPROC} ${ACCOUNT} $CONF_NBR.ksh
